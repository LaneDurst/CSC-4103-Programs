...
// 1985-2024 by Golden G. Richard III, Ph.D. (@nolaforensix)
//
...
void init_queue(Queue *q, uint32_t elementsize, bool duplicates,
	   int (*compare) (const void *e1, const void *e2), bool priority_is_tag_only) {
...
void nolock_add_to_queue(Queue *q, void *element, int32_t priority) {
  ...
  if (! q->duplicates && ! q->compare) {
    fprintf(stderr,
	    "prioque.c: If duplicates are disallowed, the comparison function must be\n"
...
bool serialize_queue(Queue *q,
  ...
  ret = (fwrite(&num_elements, sizeof(num_elements), 1, fp) == 1);
...
bool deserialize_queue(Queue *q,
  ...
  ret = (fread(&num_elements, sizeof(num_elements), 1, fp) == 1);
